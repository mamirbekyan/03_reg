...
#need to make the reps/blocks categorical variables (factor) and NOT numerical
reg_dfw
```
...
mod1_int <- lmer(yield_mgha ~1 + (1|rep),
                 data = reg_dfw)
#~ means as a function of, 1 means y is not a function of anything? Random effects for rep. Residuals help us understand if the model is a good fit for the data.
...
mod1_int_aug #keeps our response and explanatory (random effect of rep) variables, and gave us data about the residuals. Does not give standardized residual - which would have let us analyse them so we add the .stdresid column. Then we left join to get the seeding rate again??
```
...
ggplot(mod1_int_aug,
  ...
  geom_point (size = 3,
              alpha = 0.7) + #makes it a little transparent
  geom_hline(yintercept = 0,
             color = "red") + #gives us a horizontal line
  geom_smooth() #ignore the warnings
...
ggplot(mod1_int_aug,
  ...
  stat_qq() +
  stat_qq_line() #helps us understand if your residuals are normally distributed. IF they are, that's the black line - the normal distribution of the quantiles of residuals. The quantiles of the residuals of your data are the dots. Ideally, the quantiles of your residuals are going to follow the line.
...
mod1_int_randeff <- ranef(mod1_int)[[1]] #the 1 thing turned it into a dataframe
...
ggplot(mod1_int_aug, 
  ...
  geom_line(aes( y= .fixed),
            color = "blue"
            ) #This line is 12.1748, it comes from the summary above?
```
...
```{r nd}
nd <- data.frame (sr_ksha = seq(40, 120, 1)) #minimum, maximum, and in steps of size 1. Creating a dataframe that contains the values that we need to create the regression line with the confidence intervals
...
nd  <- nd %>% #this nd has the line with the mean
...
mod1_int_boots <- bootMer(mod1_int, 
                          predict.fun,
                          nsim = 200 #the actual bootstrapping. 500 is better
                          ) %>%
  confint() %>% #giving confidence interval
...
ggplot(reg_dfw,
  ...
  geom_point(size = 3,
             alpha = 0.7) +
  geom_line(data = nd, #tell it that we're taking a line from nd
            aes( y=mod1_yield_mgha),#whatever we don't specify, it will assume is the same as above
...
# Model fitting
mod2_lin <- lmer(yield_mgha ~ sr_ksha + (1|rep), #sr_ksha is our explanatory variable. 
                 data = reg_dfw
                 )
